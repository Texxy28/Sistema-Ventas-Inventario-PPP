...
<template>
    ...
    <div
        ...
        <div @click="handleSelection(null)" :class="[
        ...
        <div v-for="(category, index) in categories" :key="index" @click="handleSelection(category)" :class="[
            ...
            selectedCategory?.id_categoria === category.id_categoria
                ? 'text-white'
                : 'bg-[#B0B0AE]/40 hover:bg-[#B0B0AE]/60 text-[#2E2B26]'
        ]" :style="{
    backgroundColor:
        selectedCategory?.id_categoria === category.id_categoria
            ? categoryStyles?.[category.nombre]?.color ?? '#6B7280'
            : '',
    color:
        selectedCategory?.id_categoria === category.id_categoria
            ? 'white'
            ...
            <div class="h-full w-2 rounded-l-xl" :style="{
                backgroundColor: categoryStyles?.[category.nombre]?.color ?? '#6B7280'
            }"></div>

            <div class="flex flex-row justify-center items-center px-4 py-2">
                <component :is="categoryStyles?.[category.nombre]?.icon ?? DocumentArrowUpIcon" class="w-6 h-6" />
                <p class="font-semibold truncate text-center min-w-32">
                    {{ category.nombre }}
                </p>
            </div>
        </div>
